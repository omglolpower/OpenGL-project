...
void circle(float centerX, float centerY, float radius, float red, float green)
{
	positions[0] = centerX;
	positions[1] = centerY;
	for (int i = 0; i < 361; i++)
	...
	glUniform4f(location, red, green, 0.8f, 1.0f);
...
int main(void)
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (!check[0])
		{
			circle(flRndX, flRndY, 0.1f, 0.0f, GREEN);
		}

		// 4) check collision of circles:
		if (1.0f > (((flRndX - CX / ratio) / (R / ratio)) * ((flRndX - CX / ratio) / (R / ratio)) +
			((flRndY - CY) / R) * ((flRndY - CY) / R)))
		{
